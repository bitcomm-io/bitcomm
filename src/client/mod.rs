...
// use tokio::sync::mpsc;
use crate::object::client::DeviceConnInfo;
use crate::object::client::DeviceConnState;
use tokio::sync::Mutex;
use tokio::sync::RwLock;
use tokio::time::Instant;

static _TIME_OUT_: Duration = Duration::from_secs(120);
...
lazy_static! {
    ...
    pub static ref DEVICE_POOL: Arc<RwLock<HashMap<u64, HashMap<u32, DeviceConnInfo>>>> = Arc::new(RwLock::new(HashMap::new()));
    /// IP 地址池，键为客户端地址，值为 ClientID + DeviceID。
    pub static ref IPADRS_POOL: Arc<RwLock<HashMap<String,u128>>> = Arc::new(RwLock::new(HashMap::new()));
}
...
impl ClientPoolManager {
    ...
    pub async fn get_device_pool(clt: u64) -> Option<HashMap<u32, DeviceConnInfo>> {
        let device_pool = DEVICE_POOL.read().await;
    ...
    pub async fn remove_client(clt: u64, dev: u32) -> Option<Arc<Mutex<SendStream>>> {
        let mut device_pool = DEVICE_POOL.write().await;
    ...
    /// # 返回
    /// 返回被插入的客户端地址对应的键值。
    pub async fn put_addres(address: String, clt: u64, dev: u32) -> Option<u128> {
        let key = get_key(clt, dev);
        let mut ipadrs_pool = IPADRS_POOL.write().await;
        ipadrs_pool.insert(address, key)
    }
    ///
    pub async fn get_addres(address: &String) -> Option<u128> {
        let ipadrs_pool = IPADRS_POOL.read().await;
    ...
    pub async fn remove_addres(address: &String) -> Option<u128> {
        let mut ipadrs_pool = IPADRS_POOL.write().await;
