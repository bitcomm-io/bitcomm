...
use crate::client::ClientPoolManager;
...
pub async fn process_command_logout<'a>(
    ...
    let cdg = CommandGram::create_command_gram_from_gram(vecu8.as_mut(), reqcmdgram.as_ref());
    ...
    if let Some(ostm) =
        ClientPoolManager::remove_client(reqcmdgram.sender().into(), reqcmdgram.device_id()).await
    {
        // 关闭旧流和连接
        if !Arc::ptr_eq(&stm, &ostm) {
            let mut ostm = ostm.lock().await;
            // 清除
            ClientPoolManager::remove_addres(&ostm.connection().remote_addr().unwrap().to_string())
                .await;
